...
!!! note "注意"
    `PictionaryGame.js` と `PaintBoard.js` は `chatapp/client/src/` の中に作成する。  
...
ここからは、`chatapp/server/server.js` に Pictionary ゲーム用の処理を追加する。
...
```text
chatapp/client/src/PaintBoard.js
```
...
```text
chatapp/client/src/PictionaryGame.js
```
