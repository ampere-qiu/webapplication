...
!!! important
    ...
    提出するスクリーンショットにも、自分の名前と学籍番号が見えるようにすること。
...
### 7.5 提出用の学生情報ボックスを追加する

Step 4 の完成版では、チャット機能だけでなく、画面上部に自分の名前と学籍番号を表示する必要がある。

これは提出確認のためである。

React アプリの一番上に、次のような学生情報ボックスを追加する。
...
>
  ...
  <p><strong>Name:</strong> 自分の名前</p>
  <p><strong>Student ID:</strong> 自分の学籍番号</p>
</div>
```

`自分の名前` と `自分の学籍番号` の部分は、必ず自分の情報に書き換えること。

!!! important
    この学生情報ボックスは、チャット用の名前入力欄とは別のものである。

    チャット用の名前入力欄だけでは不十分である。

    提出前に、画面上部に名前と学籍番号が表示されていることを確認すること。
...
提出確認用の学生情報ボックスは、`return` の中で、アプリ全体を囲む一番外側の `<div>` のすぐ下に追加する。
...
また、画面上部に自分の名前と学籍番号が表示されていることも確認する。

スクリーンショットを撮るときは、次の内容が同じ画面内に見えるようにする。
...
!!! important
    スクリーンショットに名前と学籍番号が表示されていない場合、提出者を確認できない可能性がある。
...
`App.js` では、画面上部に自分の名前と学籍番号を表示する学生情報ボックスを必ず追加すること。
...
!!! warning
    ...
    提出前に、必ず画面上部の学生情報ボックスが見える状態でスクリーンショットを撮ること。
...
- `App.js` の `return` の中に学生情報ボックスを追加しているか
- 学生情報ボックスが一番外側の `<div>` の中に入っているか
- `自分の名前` と `自分の学籍番号` を自分の情報に書き換えたか
- `App.js` を保存したか
- ブラウザで `http://localhost:3000` を開いているか

スクリーンショットを撮る前に、画面上部に自分の名前と学籍番号が表示されていることを確認する。
...
| Step 2 | Reactクライアントからサーバーにメッセージを送る |
| Step 3 | 匿名チャットを作成する |
| Step 4 | 名前付きチャットを作成し、学生情報ボックスを表示する |

ミニ課題10では、Step 4 の完成版を提出する。

提出前に、画面上部に自分の名前と学籍番号が表示されていることを確認する。

スクリーンショットにも、名前と学籍番号が見えるようにする。
